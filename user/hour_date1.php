...
// CAST(loc_activities.act_timestamps AS VARCHAR(7))

    $result = pg_query($conn, " SELECT DISTINCT ON(loc_activities.act_type) loc_activities.act_type AS activity,
    to_char(TO_TIMESTAMP(loc_activities.act_timestamps), 'HH24') AS tophour, count(*) AS num_of_activity
  ...
  GROUP BY activity, tophour ORDER BY loc_activities.act_type, num_of_activity DESC");
